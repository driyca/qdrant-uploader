...
use bytes::Bytes;
use tokio::{sync::RwLock, io::Lines};
use url::Url;
...
fn make_s3_client(s3_config: &aws_config::SdkConfig) -> anyhow::Result<aws_sdk_s3::Client> { 
    let client = aws_sdk_s3::Client::new(s3_config);
    Ok(client)
}

fn make_s3_config(access_key: &str, secret_key: &str, region_name: &str, endpoint_url: &str) -> aws_config::SdkConfig {
    ...
    let region_name_cow = Cow::Owned(region_name.to_owned());
    let region = Region::new(region_name_cow);
    
    let mut sdk_config_builder = aws_config::SdkConfig::builder();
    sdk_config_builder.set_endpoint_url(Some(endpoint_url.to_owned()));
    sdk_config_builder.set_credentials_provider(Some(credential_providers));
    sdk_config_builder.set_region(region);
...
async fn open_s3_file(bucket: &str, key: &str, s3_client: &aws_sdk_s3::Client) -> anyhow::Result<Lines<BufReader<Cursor<Bytes>>>> {   
    ...
    let stream = response.body.collect().await?;
    let cursor = Cursor::new(stream.into_bytes());
